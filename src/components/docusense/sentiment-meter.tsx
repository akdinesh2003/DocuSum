      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <div className="relative w-full h-3 rounded-full bg-gradient-to-r from-red-500 via-yellow-400 to-green-500 overflow-hidden">
             <div 
                className={cn(
                  "absolute top-0 bottom-0 w-2.5 h-3.5 -mt-[1px] bg-white border border-gray-400 rounded-sm transform -translate-x-1/2 transition-all duration-500 ease-in-out",
                  {
           ...
           <p className="text-center text-lg font-semibold mt-2">{sentimentText}</p>
        </div>
        <div>
          <h4 className="font-semibold text-sm mb-1">Detected Tone</h4>
          <p className="text-muted-foreground text-sm font-mono p-2 bg-muted rounded-md">{tone || "Not available for Quick Summary"}</p>
        </div>
      </CardContent>
    </Card>
  );
